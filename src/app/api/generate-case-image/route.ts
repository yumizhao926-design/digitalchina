...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompts: Record<string, string> = {
      ...
      manufacturing: 'A clean minimalist image with a sleek car positioned exclusively on the right third of the frame, the left two-thirds must be completely empty pure white background with ABSOLUTELY NOTHING AT ALL, no elements, no objects, no shapes, no colors, no shadows, no lines, no red cube, no accent elements, absolutely blank white space, soft light gray and white gradient background only, clean and professional atmosphere, modern business style, 2K resolution',
